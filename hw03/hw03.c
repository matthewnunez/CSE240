...
void sortStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
    ...
    for (int index = 0; index < NUM_STRINGS-1; index++)
    {
        min = index;
        for (int scan = index + 1; scan < NUM_STRINGS; scan++)
        {
            if (strcmp(strings[scan], strings[min]) < 0)
            {
                min = scan;
            }
        }
        if (strings[min] != strings[index])
        {
            swapStrings(strings[min], strings[index]);
        }
    }
}
